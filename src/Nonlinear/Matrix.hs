...
import Nonlinear.V3
import Nonlinear.V4
import Nonlinear.Vector (scaled, (*^))
...
(!*!) :: (Functor m, Foldable t, Applicative t, Applicative n, Num a) => m (t a) -> t (n a) -> m (n a)
...
-- V2 50 122
(!*) :: (Applicative r, Functor m, Foldable r, Num a) => m (r a) -> r a -> m a
...
(*!) :: (Num a, Functor f, Applicative t, Num (f a), Foldable t) => t a -> t (f a) -> f a
f *! g = sum $ liftA2 (*^) f g
...
(!!/) :: (Applicative r, Fractional (r a), Functor m, Functor r, Fractional a) => m (r a) -> a -> m (r a)
m !!/ s = fmap (/ pure s) m
{-# INLINE (!!/) #-}

{- TODO blocked on Conjugate
